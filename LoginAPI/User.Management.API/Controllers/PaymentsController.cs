...
namespace API.Controllers
{
	...
	public class PaymentsController : ControllerBase
	{
		...
		private readonly FarmSubscriptionOptions _farmSubscription;
        ...
        public PaymentsController(
			...
			IOptions<FarmSubscriptionOptions> farmSubscription,
			UserManager<ApplicationUser> userManager,
			ISubscriptionService subscriberRepository,
			IUserService userService)
		{
			...
			_farmSubscription = farmSubscription.Value;
		...
		/// <summary>Farm tier bands + trial — must match frontend expectations (camelCase JSON).</summary>
		[HttpGet("subscription-tiers")]
		public IActionResult SubscriptionTiers()
		{
			var o = _farmSubscription;
			return Ok(new
			{
				trialDays = o.TrialDays,
				currency = o.Currency,
				note = "Tiers are based on total bird headcount in active flocks (sum of quantities).",
				tiers = new object[]
				{
					new { id = "tier1", maxBirds = o.Tier1MaxBirds, monthlyAmount = o.Tier1MonthlyAmount, label = $"0–{o.Tier1MaxBirds:N0} birds" },
					new { id = "tier2", maxBirds = o.Tier2MaxBirds, monthlyAmount = o.Tier2MonthlyAmount, label = $"{o.Tier1MaxBirds + 1:N0}–{o.Tier2MaxBirds:N0} birds" },
					new { id = "tier3", maxBirds = (int?)null, monthlyAmount = o.Tier3MonthlyAmount, label = $"Over {o.Tier2MaxBirds:N0} birds" },
				}
			});
		}

		/// <summary>Initialize Paystack transaction for farm subscription checkout.</summary>
		[HttpPost("initialize-paystack")]
		public async Task<IActionResult> InitializePaystack([FromBody] CreateCheckoutSessionRequest req)
		{
			if (req == null || string.IsNullOrWhiteSpace(req.SuccessUrl) || string.IsNullOrWhiteSpace(req.FailureUrl))
				return BadRequest("SuccessUrl and FailureUrl are required.");
			if (string.IsNullOrWhiteSpace(_paystackSettings.SecretKey))
				return BadRequest("Paystack secret key is not configured. Set PaystackSettings:SecretKey.");
			if (!req.TotalBirds.HasValue)
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = "Paystack checkout requires totalBirds." },
				});
			}

			var birds = req.TotalBirds.Value;
			if (birds < 0 || birds > FarmSubscriptionPricing.MaxBirdsAllowed)
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = $"totalBirds must be between 0 and {FarmSubscriptionPricing.MaxBirdsAllowed:N0}." }
				});
			}
			...
			try
			{
				quote = FarmSubscriptionPricing.GetQuote(birds, _farmSubscription);
			}
			...
			if (string.IsNullOrWhiteSpace(email))
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = "Unable to determine payer email from current session." },
				});
			}

			var amountMinor = quote.UnitAmountMinor;
			var currency = string.IsNullOrWhiteSpace(_farmSubscription.Currency) ? "GHS" : _farmSubscription.Currency.ToUpperInvariant();

			using var http = new HttpClient();
			http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _paystackSettings.SecretKey);
			...
			var response = await http.PostAsync("https://api.paystack.co/transaction/initialize", content);
			...
			if (!response.IsSuccessStatusCode)
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = $"Paystack initialize failed: {responseText}" },
				});
			}
			...
			if (!status)
			{
				var msg = root.TryGetProperty("message", out var m) ? m.GetString() : "Paystack initialize failed.";
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = msg ?? "Paystack initialize failed." },
				});
			}

			var data = root.GetProperty("data");
			var authorizationUrl = data.TryGetProperty("authorization_url", out var au) ? au.GetString() : null;
			var reference = data.TryGetProperty("reference", out var rf) ? rf.GetString() : null;
			if (string.IsNullOrWhiteSpace(authorizationUrl))
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = "Paystack did not return an authorization URL." },
				});
			}
			...
			return Ok(new CreateCheckoutSessionResponse
			{
				...
				Reference = reference,
				PublicKey = _paystackSettings.PublicKey,
			});
		}
		...
		{
			if (string.IsNullOrWhiteSpace(_stripeSettings.PrivateKey))
				return BadRequest("Stripe private key is not configured. Set StripeSettings:PrivateKey.");
			...
			else if (!string.IsNullOrWhiteSpace(req.PriceId))
			{
				options = new SessionCreateOptions
				{
					SuccessUrl = req.SuccessUrl,
			...
			else
			{
				return BadRequest(new ErrorResponse
				{
					ErrorMessage = new ErrorMessage { Message = "Provide either priceId or totalBirds." },
				});
			}
